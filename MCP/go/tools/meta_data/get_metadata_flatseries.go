...
func Get_metadata_flatseriesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/metadata/flatseries%s", cfg.BaseURL, queryString)
...
func CreateGet_metadata_flatseriesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_metadata_flatseries",
		mcp.WithDescription("Returns a list of flattened metadata on the patient, study and series levels"),
		mcp.WithNumber("startindex", mcp.Description("start index of returned slice of flat series")),
		mcp.WithNumber("count", mcp.Description("size of returned slice of flat series")),
		mcp.WithString("orderby", mcp.Description("flat series property to order results by")),
		mcp.WithBoolean("orderascending", mcp.Description("order result ascendingly if true, descendingly otherwise")),
		mcp.WithString("filter", mcp.Description("filter the results by matching substrings of flat series properties against this value")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_metadata_flatseriesHandler(cfg),
	}
}
