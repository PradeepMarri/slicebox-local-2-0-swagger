...
func Get_metadata_studiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["patientid"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("patientid=%v", val))
		}
		...
		url := fmt.Sprintf("%s/metadata/studies%s", cfg.BaseURL, queryString)
...
func CreateGet_metadata_studiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_metadata_studies",
		mcp.WithDescription("Returns a list of metadata on the study level of the DICOM hierarchy"),
		mcp.WithNumber("startindex", mcp.Description("start index of returned slice of studies")),
		mcp.WithNumber("count", mcp.Description("size of returned slice of studies")),
		mcp.WithNumber("patientid", mcp.Required(), mcp.Description("reference to patient to list studies for")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_metadata_studiesHandler(cfg),
	}
}
