...
func Delete_metadata_series_seriesid_seriestypes_seriestypeidHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDelete_metadata_series_seriesid_seriestypes_seriestypeidTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_metadata_series_seriesId_seriestypes_seriesTypeId",
		mcp.WithDescription("Delete the series type with the supplied series type ID from the series with the supplied series ID"),
		mcp.WithNumber("seriesId", mcp.Required(), mcp.Description("ID of series")),
		mcp.WithNumber("seriesTypeId", mcp.Required(), mcp.Description("ID of series type to remove")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Delete_metadata_series_seriesid_seriestypes_seriestypeidHandler(cfg),
	}
}
