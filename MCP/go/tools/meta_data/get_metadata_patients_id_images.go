...
func Get_metadata_patients_id_imagesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/metadata/patients/%s/images%s", cfg.BaseURL, id, queryString)
...
func CreateGet_metadata_patients_id_imagesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_metadata_patients_id_images",
		mcp.WithDescription("Returns all images for the patient with the supplied patient ID"),
		mcp.WithNumber("id", mcp.Required(), mcp.Description("ID of patient")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_metadata_patients_id_imagesHandler(cfg),
	}
}
